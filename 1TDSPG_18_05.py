...
#faça uma função que recebe uma lista de numeros e retorna o indice do maior numero
...
def cadastra_preco(precos,qtd):
    for i in range(qtd):
        nome = input("Diga o preço do vinho : ")
        precos.append(nome)
    return precos
...
def verifica_opcoes(msg,opcoes):
    ...
    while not opcao in opcoes:
        print("Voce deve digitar uma opcao válida!")
        opcao = input(msg)
    return opcao
...
#faça uma função que pede numeros ao usuario e retorna o numero somente quando for um int
#faça uma função que acha o preco medio dos vinhos
